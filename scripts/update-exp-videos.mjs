...
for (const [expId, videos] of Object.entries(videoMapping)) {
  ...
  const videoLines = videos.map(v => `      { title: '${v.title}', url: '${v.url}' }`).join(',\n');
  const videosStr = `videos: [\n${videoLines}\n    ]`;
  
  const regex = new RegExp(`id:\\s*'${expId}',[\\s\\S]*?videoUrl:\\s*'[^']*',`);
  const match = content.match(regex);
  
  if (match) {
    const oldStr = match[0];
    const newStr = oldStr.replace(/videoUrl:\s*'[^']*',/, videosStr + ',');
    content = content.replace(oldStr, newStr);
    console.log('✓ 更新实验: ' + expId + ' (' + videos.length + '个视频)');
    updatedCount++;
  } else {
