...
const tempWmfDir = 'temp-wmf-final';
...
async function step1() {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      let localImgIdx = 0;
      const localImgInfo = []; // { placeholder, type, wmfName }
          ...
          convertImage: mammoth.images.imgElement(function(image) {
            return image.read().then(function(buf) {
              ...
              if (isPng && buf.length > 10000) {
                ...
                type = 'formula';
                wmfCounter++;
                wmfName = `formula_${wmfCounter}.wmf`;
                fs.writeFileSync(path.join(tempWmfDir, wmfName), buf);
              }
              
              localImgInfo.push({ placeholder, type, wmfName, bufSize: buf.length });
      ...
      const figureImgs = localImgInfo.filter(i => i.type === 'figure')
        .map(i => `/question-images/${experimentId}-${parsed.difficulty}-fig-${i.placeholder.match(/_(\d+)__$/)[1]}.png`);
      ...
      for (const q of questions) {
        ...
        allQuestions.push({
          ...
          hasFormula: q.html.includes('formula') || ansHtml.includes('formula') || detHtml.includes('formula')
        });
      }
      
      console.log(`✓ ${questions.length}题, ${localImgInfo.filter(i=>i.type==='formula').length}公式`);
  ...
  // 保存中间数据
  fs.writeFileSync('temp-questions-step1.json', JSON.stringify(allQuestions, null, 2));
  console.log(`\n第一步完成：`);
  console.log(`  总题数: ${allQuestions.length}`);
  console.log(`  WMF文件: ${wmfCounter}`);
  console.log(`  中间数据: temp-questions-step1.json`);
